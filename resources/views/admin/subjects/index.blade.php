<x-app-layout>
    <x-slot name="header">
        {{ __('Manajemen Mata Pelajaran (Subjects)') }}
    </x-slot>

    <!-- MD3 + Glassmorphism Container -->
    <div class="bg-white/60 backdrop-blur-2xl border border-white/40 rounded-[2.5rem] shadow-2xl overflow-hidden relative" x-data="{ openModal: false, editMode: false, editId: null, editName: '', editLevelId: '' }">
        ...
        <div class="p-8 flex flex-col md:flex-row justify-between items-center gap-6 relative z-10">
            <!-- Search Bar -->
            <form action="{{ route('subjects.index') }}" method="GET" class="w-full md:w-1/3 relative group">
            ...
            <div class="flex items-center gap-4">
                <button @click="openModal = true; editMode = false; editName = ''; editLevelId = '{{ $levels->first()->id ?? '' }}'" class="px-8 py-4 rounded-xl bg-emerald-600 text-white font-bold shadow-xl shadow-emerald-600/20 hover:bg-emerald-700 hover:shadow-emerald-600/30 hover:scale-[1.02] transition-all flex items-center gap-3">
        ...
        <div class="px-4 pb-8 relative z-10">
                    ...
                    <div class="group bg-white/70 backdrop-blur-xl border border-white/60 p-5 rounded-[1.5rem] shadow-sm hover:shadow-[0_8px_30px_rgb(0,0,0,0.04)] hover:bg-white hover:scale-[1.01] transition-all duration-300 relative overflow-hidden">
                        ...
                        <div class="flex justify-between items-start mb-4">
                            ...
                            <div class="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity transform translate-x-2 group-hover:translate-x-0">
                                <button @click="openModal = true; editMode = true; editId = {{ $subject->id }}; editName = '{{ $subject->name }}'; editLevelId = '{{ $subject->level_id }}'" class="p-2 rounded-full bg-amber-50 text-amber-600 hover:bg-amber-100 transition shadow-sm" title="Edit">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"></path></svg>
                                </button>
                                <form action="{{ route('subjects.destroy', $subject->id) }}" method="POST" onsubmit="return confirm('Hapus mata pelajaran ini?');">
                                    @csrf @method('DELETE')
                        ...
                        <!-- Card Content -->
                        <div>
                            <h3 class="text-xl font-bold text-gray-800 leading-tight mb-1">{{ $subject->name }}</h3>
                            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold bg-emerald-100 text-emerald-800 mb-4 border border-emerald-200">
                                {{ $subject->level->name }}
                            </span>
                            
                            <div class="text-xs">
                                <span class="text-gray-400 font-bold block">PEMAKAIAN SESI UJIAN</span>
                                <span class="text-emerald-700 font-extrabold text-sm">{{ $subject->examSessions()->count() }} Sesi</span>
                            </div>
                        </div>
                    </div>
                    @endforeach
            ...
            @else
                <div class="flex flex-col items-center justify-center py-20 text-center opacity-60">
                    ...
                    <h3 class="text-xl font-bold text-gray-600">Belum ada Mata Pelajaran</h3>
                    <p class="text-gray-500 mt-1 max-w-sm">Tambahkan mata pelajaran baru dan hubungkan ke tingkatan kelas.</p>
                </div>
            @endif
        </div>
            ...
            <div x-show="openModal" class="fixed inset-0 z-[100] flex items-center justify-center p-4" style="display: none;">
                ...
                <div x-show="openModal"
                    ...
                    <div class="h-32 bg-gradient-to-br from-emerald-500 to-teal-600 relative overflow-hidden flex items-end p-8">
                        ...
                        <h3 class="text-2xl font-bold text-white relative z-10 tracking-tight" x-text="editMode ? 'Edit Mapel' : 'Mapel Baru'"></h3>
                    ...
                    <div class="p-8">
                        <form :action="editMode ? '{{ url('subjects') }}/' + editId : '{{ route('subjects.store') }}'" method="POST">
                            @csrf
                            ...
                            <div class="space-y-6">
                                <div class="group">
                                    <label class="block text-xs font-bold text-emerald-800 uppercase tracking-widest mb-2 ml-1">Tingkatan</label>
                                    <select name="level_id" x-model="editLevelId"
                                ...
                                <div class="group">
                                    <label class="block text-xs font-bold text-emerald-800 uppercase tracking-widest mb-2 ml-1">Nama Mata Pelajaran</label>
