<x-app-layout>
    <x-slot name="header">
        {{ __('Manajemen Sesi Waktu (Time Sessions)') }}
    </x-slot>

    <!-- MD3 + Glassmorphism Container -->
    <div class="bg-white/60 backdrop-blur-2xl border border-white/40 rounded-[2.5rem] shadow-2xl overflow-hidden relative" x-data="{ openModal: false, editMode: false, editId: null, editName: '', editStartTime: '', editEndTime: '' }">
        ...
        <div class="p-8 flex flex-col md:flex-row justify-between items-center gap-6 relative z-10">
            <!-- Search Bar -->
            <form action="{{ route('time_sessions.index') }}" method="GET" class="w-full md:w-1/3 relative group">
            ...
            <div class="flex items-center gap-4">
                <button @click="openModal = true; editMode = false; editName = ''; editStartTime = '07:30'; editEndTime = '09:30'" class="px-8 py-4 rounded-xl bg-emerald-600 text-white font-bold shadow-xl shadow-emerald-600/20 hover:bg-emerald-700 hover:shadow-emerald-600/30 hover:scale-[1.02] transition-all flex items-center gap-3">
        ...
        <div class="px-4 pb-8 relative z-10">
                    ...
                    <div class="group bg-white/70 backdrop-blur-xl border border-white/60 p-5 rounded-[1.5rem] shadow-sm hover:shadow-[0_8px_30px_rgb(0,0,0,0.04)] hover:bg-white hover:scale-[1.01] transition-all duration-300 relative overflow-hidden">
                        ...
                        <div class="flex justify-between items-start mb-4">
                            ...
                            <div class="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity transform translate-x-2 group-hover:translate-x-0">
                                <button @click="openModal = true; editMode = true; editId = {{ $ts->id }}; editName = '{{ $ts->name }}'; editStartTime = '{{ substr($ts->start_time, 0, 5) }}'; editEndTime = '{{ substr($ts->end_time, 0, 5) }}'" class="p-2 rounded-full bg-amber-50 text-amber-600 hover:bg-amber-100 transition shadow-sm" title="Edit">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"></path></svg>
                                </button>
                                <form action="{{ route('time_sessions.destroy', $ts->id) }}" method="POST" onsubmit="return confirm('Hapus sesi waktu ini?');">
                                    @csrf @method('DELETE')
                        ...
                        <!-- Card Content -->
                        <div>
                            <h3 class="text-xl font-bold text-gray-800 leading-tight mb-2">{{ $ts->name }}</h3>
                            <div class="flex items-center gap-2 mb-4">
                                <span class="px-3 py-1 rounded-xl bg-teal-50 border border-teal-100 text-teal-700 font-extrabold text-xs">
                                    {{ substr($ts->start_time, 0, 5) }}
                                </span>
                                <span class="text-gray-400 font-extrabold text-xs">S/D</span>
                                <span class="px-3 py-1 rounded-xl bg-teal-50 border border-teal-100 text-teal-700 font-extrabold text-xs">
                            ...
                            <div class="text-xs">
                                <span class="text-gray-400 font-bold block">PEMAKAIAN SESI UJIAN</span>
                                <span class="text-emerald-700 font-extrabold text-sm">{{ $ts->examSessions()->count() }} Sesi</span>
                            </div>
                        </div>
                    </div>
                    @endforeach
            ...
            @else
                <div class="flex flex-col items-center justify-center py-20 text-center opacity-60">
                    ...
                    <h3 class="text-xl font-bold text-gray-600">Belum ada Sesi Waktu</h3>
                    <p class="text-gray-500 mt-1 max-w-sm">Tambahkan sesi waktu baru (misal Sesi 1: 07:30 - 09:30).</p>
                </div>
            @endif
        </div>
            ...
            <div x-show="openModal" class="fixed inset-0 z-[100] flex items-center justify-center p-4" style="display: none;">
                ...
                <div x-show="openModal"
                    ...
                    <div class="h-32 bg-gradient-to-br from-emerald-500 to-teal-600 relative overflow-hidden flex items-end p-8">
                        ...
                        <h3 class="text-2xl font-bold text-white relative z-10 tracking-tight" x-text="editMode ? 'Edit Sesi Waktu' : 'Sesi Waktu Baru'"></h3>
                    ...
                    <div class="p-8">
                        <form :action="editMode ? '{{ url('time_sessions') }}/' + editId : '{{ route('time_sessions.store') }}'" method="POST">
                            @csrf
                            ...
                            <div class="space-y-6">
                                <div class="group">
                                    <label class="block text-xs font-bold text-emerald-800 uppercase tracking-widest mb-2 ml-1">Nama Sesi</label>
                                    <input type="text" name="name" x-model="editName"
                                           ...
                                           placeholder="Cth: Sesi 1" required autofocus>
                                </div>

                                <div class="grid grid-cols-2 gap-4">
                                    <div class="group">
                                        <label class="block text-xs font-bold text-emerald-800 uppercase tracking-widest mb-2 ml-1">Jam Mulai</label>
                                        <input type="time" name="start_time" x-model="editStartTime"
                                               class="w-full bg-gray-50/80 border-0 rounded-2xl p-4 text-gray-900 font-medium focus:bg-white focus:ring-2 focus:ring-emerald-500 transition-all shadow-inner" required>
                                    </div>
                                    <div class="group">
                                        <label class="block text-xs font-bold text-emerald-800 uppercase tracking-widest mb-2 ml-1">Jam Selesai</label>
                                        <input type="time" name="end_time" x-model="editEndTime"
                                               class="w-full bg-gray-50/80 border-0 rounded-2xl p-4 text-gray-900 font-medium focus:bg-white focus:ring-2 focus:ring-emerald-500 transition-all shadow-inner" required>
                                    </div>
                                </div>
                            </div>
